...
export function validateLevelTask(levelId: number, s: AvatarState): { ok: true } | { ok: false; reason: string } {
  switch (levelId) {
    ...
    case 21:
      return diffCount(s, DEFAULT_AVATAR) >= 10
        ? { ok: true }
        : { ok: false, reason: 'Соберите образ с 10+ отличиями от базового.' };
    case 22:
      return s.casualJewelry !== 'none' && s.partDecorFace !== 'none' && hasAnyMakeup(s)
        ? { ok: true }
        : { ok: false, reason: 'Украшение + декор лица + макияж.' };
    case 23: {
      const d = Number.parseInt(s.casualDress, 10);
      const dressOk = s.outfitMode === 'dress' && Number.isFinite(d) && d >= 5;
      return dressOk && HEELS.has(s.casualShoes)
        ? { ok: true }
        : { ok: false, reason: 'Платье (не ниже №5), режим платья и каблуки.' };
    }
    case 24:
      return (
        s.outfitMode === 'separate' &&
        s.casualTop !== '1' &&
        s.casualBottom !== '1' &&
        SNEAKERS.has(s.casualShoes) &&
        hasAnyMakeup(s)
      )
        ? { ok: true }
        : { ok: false, reason: 'Верх и низ не базовые, кроссовки и лёгкий макияж.' };
    case 25: {
      const epic =
        diffCount(s, DEFAULT_AVATAR) >= 12 &&
        makeupLayerCount(s) >= 4 &&
        hasAnyMakeup(s) &&
        s.outfitMode === 'dress' &&
        HEELS.has(s.casualShoes) &&
        s.casualJewelry !== 'none' &&
        s.casualJacket !== 'none';
      return epic ? { ok: true } : { ok: false, reason: 'Финал сезона: платье, каблуки, жакет, украшение, богатый макияж (4+ слоя) и 12+ отличий.' };
    }
    default:
